...
import { jobAPI } from '../services/apiService'; // Adjust path as needed

// Icon mapping function
const getIconForType = (title) => {
  const titleLower = title.toLowerCase();
  if (titleLower.includes('frontend') || titleLower.includes('react')) return Briefcase;
  if (titleLower.includes('backend') || titleLower.includes('node')) return Briefcase;
  if (titleLower.includes('design') || titleLower.includes('ui/ux')) return Briefcase;
  return Briefcase;
};
...
export default function JobRecommend() {
  ...
  console.log('userId from URL params:', userId); // Debug log
  ...
  const fetchRecommendedJobs = async () => {
    ...
    try {
      ...
      console.log('Fetching jobs for userId:', userId); // Debug log
      ...
      setError(err.message || 'Failed to load recommended jobs. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSkillGapAnalysis = async (job) => {
    ...
    try {
      ...
      alert(err.message || 'Failed to generate skill gap analysis. Please try again.');
      setSelectedJob(null);
    } finally {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <Loader2 className="h-16 w-16 animate-spin mx-auto mb-4" style={{ color: '#048998' }} />
          <p className="text-gray-600 text-lg font-medium">Loading recommended jobs...</p>
          <p className="text-gray-500 text-sm mt-2">Finding the best matches for your skills</p>
        </div>
      </div>
    );
  }
      ...
      <div>
        ...
        {jobs.length === 0 ? (
          <div className="text-center py-16 px-4">
            ...
            <h3 className="text-2xl font-semibold text-gray-700 mb-3">No matching jobs found</h3>
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              We couldn't find any jobs matching your current skills. Try updating your profile or check back later for new opportunities.
            </p>
            <button
              onClick={fetchRecommendedJobs}
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {job.matchedSkills && job.matchedSkills.length > 0 ? (
                          ...
                          <span className="text-xs text-gray-500">No matched skills</span>
                        )}
                      </div>
                    </div>

                    {/* Skill Gap Analysis Button - Show for partial matches */}
                    {isPartialMatch && (
                      <button
                        ...
                        style={{
                          background: loadingSkillGap && selectedJob?._id === job._id ? '#fef3c7' : '#fef3c7',
                          color: '#92400e',
                          borderColor: '#fbbf24'
                        }}
                        onClick={() => handleSkillGapAnalysis(job)}
                        disabled={loadingSkillGap && selectedJob?._id === job._id}
                      >
